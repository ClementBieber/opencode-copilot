...
- **Native tracking**: Some platforms (e.g., OpenCode) natively record cost, tokens, and model info per assistant message. Avoid duplicating what the platform already captures — layer budget controls on top instead.

### Request-based billing models
Some providers (e.g., GitHub Copilot) bill by "requests" and "premium requests" rather than per-token. In these environments:
- Native cost tracking may show $0.00 per message since there is no per-token charge.
- Budget controls should count requests and premium requests instead of (or in addition to) tokens/USD.
- Model routing decisions still apply — routing to a non-premium model saves premium request quota.
- The skill's primary value in request-based billing is in **routing decisions** and **quota management**, not cost-per-token tracking.
