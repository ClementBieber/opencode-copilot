...
### Data source: OpenCode session database

When running inside OpenCode, the extractor should query the native session database rather than assuming raw text transcripts:

- **Location**: `~/.local/share/opencode/opencode.db` (SQLite, WAL mode)
- **Schema**: `project` → `session` → `message` → `part` (hierarchical)
- **Message format**: The `message.data` column contains typed JSON blobs — `UserMessage` or `AssistantMessage` — with full content, tool calls, and metadata.
- **Cost metadata**: Each `AssistantMessage` includes `cost`, `tokens` (input/output/reasoning/cache), `modelID`, and `providerID`.
- **Hook point**: The `session.updated` event can trigger extraction. For batch processing, query sessions by `updated_at` timestamp.
- **Project scoping**: `Session.list()` filters by `project_id`. For cross-project extraction, use the `GET /experimental/session` API or query the DB directly.

The extractor pipeline becomes:

```
extract_patterns(session_id, db_path):
  messages = query_messages(db_path, session_id)
  redacted = redact(messages)
  units = split_into_units(redacted)
  candidates = []
  for u in units:
    issues = detect_issues(u)
    for issue in issues:
      p = summarize(issue, u, session_metadata)
...
### Generic extractor (non-OpenCode)

For platforms without a queryable session database, use raw transcripts:
